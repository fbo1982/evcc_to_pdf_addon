...
def inject_common():
    ...
    return {"settings": settings, "group_count": len(settings.get("groups", [])), "vehicle_count": len(settings.get("cached_vehicles", []))}
...
def report_page():
    ...
    return render_template("report.html", settings=settings, years=years, months=months, generated_file=generated_file)
